...
find "$PUBLIC_DIR" -maxdepth 1 -type f -name '*.xbps' -printf '%f\n' | sort | while IFS= read -r f; do
  base="${f%.x86_64.xbps}"
  pkgname="${base%-*}"
  pkgver="${base##*-}"

  desc=$(grep -E '^short_desc=' "pkgs/$pkgname/template" 2>/dev/null | head -1 | sed "s/^short_desc=\"//;s/\"$//")

  cat >> "$OUT" << ITEM
        <a class="pkg-item" href="$f">
